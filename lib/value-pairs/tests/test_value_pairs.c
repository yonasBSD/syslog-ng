...
ParameterizedTestParameters(value_pairs, test)
{
  static const struct value_pairs_params params[] =
  {
    ...
    {"rfc3164", "test.*", TRUE, "DATE,FACILITY,HOST,MESSAGE,PID,PRIORITY,PROGRAM", NULL },

    /* tests that excluding works even when the key would be in the
     * default set. */
    {"nv-pairs", "MESSAGE", TRUE, "HOST,MSGFORMAT,MSGID,PID,PROGRAM", NULL }
...
{
  testcase(param->scope, param->has_exclude ? param->exclude : NULL, param->expected);
}
